...
namespace AX.Core
{
    ...
    public class FTP
    {
        ...
        public FTP(string serverIp)
        {
            serverIp.CheckIsNullOrWhiteSpace();
            ServerIP = serverIp;
        }

        public FTP(string serverIp, string loginname, string password)
        {
            ServerIP = serverIp;
            LoginName = loginname;
            Password = password;
        ...
        /// </summary>
        /// <param name="ftpFileName">远程文件名</param>
        ...
        public void DownloadFile(string ftpFileName, string localFileName = null)
        {
            ServerIP.CheckIsNullOrWhiteSpace();
            if (string.IsNullOrWhiteSpace(localFileName))
            { localFileName = ftpFileName.Split("/").Last().ToString(); }

            var outputStream = new FileStream(localFileName, FileMode.Create);
            var ftpFileUrl = new Uri("ftp://" + ServerIP + "/" + ftpFileName);

            //获取文件大小
            //var ftpsize = (FtpWebRequest)WebRequest.Create(ftpFileUrl);
            //ftpsize.UseBinary = true;
            //ftpsize.Credentials = new NetworkCredential(LoginName, Password);
            //ftpsize.Method = WebRequestMethods.Ftp.GetFileSize;
            //var re = (FtpWebResponse)ftpsize.GetResponse();
            //long totalBytes = re.ContentLength;
            //

            var ftpConn = (FtpWebRequest)WebRequest.Create(ftpFileUrl);
            ftpConn.UseBinary = true;
            ftpConn.KeepAlive = false;
            ftpConn.Credentials = new NetworkCredential(LoginName, Password);
            ftpConn.Method = WebRequestMethods.Ftp.DownloadFile;
            var response = (FtpWebResponse)ftpConn.GetResponse();
        ...
        public void BrokenDownload(string remoteFileName, string localFileName, bool ifCredential, long size, Action<int, int> updateProgress = null)
        {
            ...
            Uri uri = new Uri("ftp://" + ServerIP + "/" + remoteFileName);
        ...
        public void Download(string remoteFileName, string localFileName, bool ifCredential, bool brokenOpen, Action<int, int> updateProgress = null)
        {
            ...
            DownloadFile(remoteFileName, localFileName, ifCredential, updateProgress);
        }
        ...
        public void UploadFile(string relativePath, string localFullPathName, Action<int, int> updateProgress = null)
        {
            ...
            Uri uri = new Uri("ftp://" + ServerIP + "/" + relativePath + "/" + finfo.Name);
            var reqFtp = (FtpWebRequest)WebRequest.Create(uri);
            reqFtp.KeepAlive = false;
            reqFtp.UseBinary = true;
            reqFtp.Credentials = new NetworkCredential(LoginName, Password); //用户，密码
        ...
        public bool UploadBroken(string localFullPath, string remoteFilepath, Action<int, int> updateProgress = null)
        {
            ...
            {
                uri = "ftp://" + ServerIP + "/" + newFileName;
            }
            else
            {
                uri = "ftp://" + ServerIP + "/" + remoteFilepath + "/" + newFileName;
            }
        ...
        public long GetFileSize(string filePath, string remoteFilepath)
        {
            try
            {
                ...
                if (remoteFilepath.Length == 0)
                {
                    uri = "ftp://" + ServerIP + "/" + fi.Name;
                }
                else
                {
                    uri = "ftp://" + ServerIP + "/" + remoteFilepath + "/" + fi.Name;
                }
        ...
        public List<string> GetFilesDetails(string relativePath = "")
        {
            ...
            var ftp = (FtpWebRequest)WebRequest.Create(new Uri(Path.Combine("ftp://" + ServerIP, relativePath).Replace("\\", "/")));
            ftp.Credentials = new NetworkCredential(LoginName, Password);
        ...
        {
            var result = new List<string>();
            var reqFtp = (FtpWebRequest)WebRequest.Create(new Uri(Path.Combine("ftp://" + ServerIP, relativePath).Replace("\\", "/")));
            reqFtp.UseBinary = true;
            reqFtp.Credentials = new NetworkCredential(LoginName, Password);
        ...
        {
            string uri = Path.Combine("ftp://" + ServerIP, filePath).Replace("\\", "/");
            var reqFtp = (FtpWebRequest)WebRequest.Create(new Uri(uri));
            reqFtp.Credentials = new NetworkCredential(LoginName, Password);
            reqFtp.KeepAlive = false;
        ...
        {
            string uri = Path.Combine("ftp://" + ServerIP, dirPath).Replace("\\", "/");
            var reqFtp = (FtpWebRequest)WebRequest.Create(new Uri(uri));
            reqFtp.Credentials = new NetworkCredential(LoginName, Password);
            reqFtp.KeepAlive = false;
        ...
        {
            var reqFtp = (FtpWebRequest)WebRequest.Create(new Uri(Path.Combine("ftp://" + ServerIP, filePath).Replace("\\", "/")));
            reqFtp.Method = WebRequestMethods.Ftp.GetFileSize;
            reqFtp.UseBinary = true;
            reqFtp.Credentials = new NetworkCredential(LoginName, Password);
            using var response = (FtpWebResponse)reqFtp.GetResponse();
        ...
        {
            var reqFtp = (FtpWebRequest)WebRequest.Create(new Uri(Path.Combine("ftp://" + ServerIP, relativePath, newDir).Replace("\\", "/")));
        ...
        {
            var reqFtp = (FtpWebRequest)WebRequest.Create(new Uri(Path.Combine("ftp://" + ServerIP, relativePath, currentFilename).Replace("\\", "/")));
