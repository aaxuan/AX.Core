...
namespace AX.Core.RunLog
{
    ...
    // ///
    // <summary>
    // /// 实例化 文件日志记录 /// 路径不传则取当前目录 ///
    // </summary>
    // ///
    // <param name="logDirectoryPath"> </param>
    // public FileLoger(string logDirectoryPath = null) { //AppDomain.CurrentDomain.BaseDirectory
    // D:\MyProject\AX.Core\NetCoreUseDemo\bin\Debug\netcoreapp3.1\ if
    // (string.IsNullOrWhiteSpace(logDirectoryPath)) { logDirectoryPath =
    ...
    // ///// <summary> ///// 日志队列 ///// </summary> //private Queue<string> Msgs;
    ...
    // ////日志文件写入线程执行的方法 //private void work() //{ // while (true) // { // //判断队列中是否存在待写入的日志 // if
    ...
    // writer.Write('\t'); // writer.Write(msg.Type); // writer.Write('\t'); //
    // writer.WriteLine(msg.Text); // writer.Flush(); // } // } // catch (Exception e) // { //
    // Console.Out.Write(e); // } //}

    // ////打开文件准备写入 //private void FileOpen() //{ // writer = new StreamWriter(path + GetFilename(),
    // true, Encoding.UTF8); //}

    // ////关闭打开的日志文件 //private void FileClose() //{ // if (writer != null) // { // writer.Flush();
    // // writer.Close(); // writer.Dispose(); // writer = null; // } //}

    // public override void Info(string msg) { WriteFileLog("[信息] " + msg); }

    // public override void Err(string msg) { WriteFileLog("[异常] " + msg); }

    // public override void Waring(string msg) { WriteFileLog("[警告] " + msg); }

    // public override void Line() { WriteFileLog($"---------- ---------- ---------- ----------
    // ---------- ----------"); }
